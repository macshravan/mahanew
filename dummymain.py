...
def admin():
    ...
    def mark_checkin():
        ...
        if selected_items:
            ...
            data = load_data()
            data[index+1][-1] = "Checkedin"  # +1 because the first row is header
            update_csv(data)
            refresh_display()
    ...
    def checkout():
        ...
        with open("request.csv", mode="a", newline="") as file:
            ...
            for item in cart_items:
                category, name, cost, quantity, food_id = item
                writer.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), category, name, quantity, cost, current_user_email, current_user_username,"booked"])
